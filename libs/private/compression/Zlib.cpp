...
#include "compression/UiDeflate.h"
#include "compression/Zlib.h"

std::uint32_t SferaUiDeflateBits::read(unsigned count)
{
    std::uint32_t result = 0u;
    for (unsigned bit = 0u; bit < count; ++bit)
    {
        if (position == input.size())
            throw std::runtime_error("Truncated UI configuration stream");
...
SferaUiDeflateAlphabet::SferaUiDeflateAlphabet(std::span<const std::uint8_t> lengths, bool require_complete)
{
    for (const auto length : lengths)
    {
        if (length > 15u)
            throw std::runtime_error("Invalid UI Huffman length");
        ++counts[length];
    }
    int available = 1;
    for (unsigned length = 1u; length < 16u; ++length)
    {
        ...
        if (available < 0)
            throw std::runtime_error("Oversubscribed UI Huffman alphabet");
    }
    if (available != 0 && (require_complete || *std::max_element(lengths.begin(), lengths.end()) > 1u))
        throw std::runtime_error("Incomplete UI Huffman alphabet");
    std::array<std::uint16_t, 16> offsets{};
    for (unsigned length = 1u; length < 15u; ++length)
        offsets[length + 1u] = offsets[length] + counts[length];
    for (std::uint16_t symbol = 0u; symbol < lengths.size(); ++symbol)
        if (lengths[symbol] != 0u)
            symbols[offsets[lengths[symbol]]++] = symbol;
}

std::uint32_t SferaUiDeflateAlphabet::decode(SferaUiDeflateBits &bits) const
{
...
{
    if (input.size() < 6u || (input[0] & 15u) != 8u || (input[0] >> 4u) > 7u || ((input[0] * 256u + input[1]) % 31u) != 0u || (input[1] & 32u) != 0u)
        throw std::runtime_error("Invalid UI configuration zlib header");
    SferaUiDeflateBits bits{input.subspan(2u)};
    ...
    do
    {
        ...
        if (kind == 0u)
        {
            ...
            if ((length ^ complement) != 65535u)
                throw std::runtime_error("Invalid UI stored block length");
            for (unsigned index = 0u; index < length; ++index)
                appendUiByte(output, limit, static_cast<std::uint8_t>(bits.read(8u)));
            continue;
        }
        if (kind == 3u)
            throw std::runtime_error("Invalid UI deflate block");
        std::array<std::uint8_t, 288> literal_lengths{};
        std::array<std::uint8_t, 32> distance_lengths{};
        if (kind == 1u)
        {
            for (unsigned index = 0u; index < literal_lengths.size(); ++index)
                literal_lengths[index] = SferaDeflateState32::fixedLiteralLength(index);
        ...
        else
        {
            ...
            if (literals > 286u)
                throw std::runtime_error("Invalid UI literal alphabet");
            std::array<std::uint8_t, 19> code_lengths{};
            for (unsigned index = 0u; index < codes; ++index)
                code_lengths[SferaZlibTables::codeLengthOrder[index]] = static_cast<std::uint8_t>(bits.read(3u));
            SferaUiDeflateAlphabet alphabet{code_lengths, true};
            ...
            while (lengths.size() < literals + distances)
            {
                ...
                if (code < 16u)
                    lengths.push_back(static_cast<std::uint8_t>(code));
                else
                {
                    if (code == 16u && lengths.empty())
                        throw std::runtime_error("UI Huffman repeat without predecessor");
                    const auto count = code == 16u ? bits.read(2u) + 3u : code == 17u ? bits.read(3u) + 3u : bits.read(7u) + 11u;
                    if (count > literals + distances - lengths.size())
                        throw std::runtime_error("UI Huffman repeat overflow");
        ...
        if (literal_lengths[256] == 0u)
            throw std::runtime_error("UI deflate block has no terminator");
        SferaUiDeflateAlphabet literals{literal_lengths}, distances{distance_lengths};
        ...
        constexpr unsigned distance_base[] = {1u,   2u,   3u,   4u,   5u,   7u,    9u,    13u,   17u,   25u,   33u,   49u,   65u,    97u,    129u,
        ...
        for (;;)
        {
            ...
            if (symbol < 256u)
            {
                appendUiByte(output, limit, static_cast<std::uint8_t>(symbol));
                continue;
            }
            if (symbol > 285u)
                throw std::runtime_error("Invalid UI length symbol");
            ...
            if (distance_symbol >= 30u)
                throw std::runtime_error("Invalid UI distance symbol");
            ...
            if (distance > output.size())
                throw std::runtime_error("UI deflate distance before buffer");
            for (unsigned index = 0u; index < length; ++index)
                appendUiByte(output, limit, output[output.size() - distance]);
        }
    } while (!last);
    bits.align();
    std::uint32_t checksum = 0u;
    for (unsigned index = 0u; index < 4u; ++index)
        checksum = (checksum << 8u) | bits.read(8u);
    ...
    if (checksum != ((second << 16u) | first))
        throw std::runtime_error("UI configuration checksum mismatch");
    return output;
}
...
SferaDeflateConfig SferaDeflateState32::deflateConfig(int32_t level)
{
    ...
    case 2:
        return {4u, 5u, 16u, 8u};
    case 3:
        return {4u, 6u, 32u, 32u};
    case 4:
        return {4u, 4u, 16u, 16u};
    case 5:
        return {8u, 16u, 32u, 32u};
    case 6:
        return {8u, 16u, 128u, 128u};
    case 7:
        return {8u, 32u, 128u, 256u};
    case 8:
        return {32u, 128u, 258u, 1024u};
    case 9:
        return {32u, 258u, 258u, 4096u};
    default:
        return {0u, 0u, 0u, 0u};
    }
}
...
uint16_t SferaDeflateState32::fixedLiteralCode(uint32_t symbol)
{
    if (symbol <= 143u)
    {
        return reverseBits(symbol + fixed_literal_start, 8u);
    }
    if (symbol <= 255u)
    {
        return reverseBits(symbol - 144u + fixed_high_start, 9u);
    }
    if (symbol <= 279u)
    {
        return reverseBits(symbol - 256u, 7u);
    }
    if (symbol <= 287u)
    {
        return reverseBits(symbol - 280u + fixed_tail_start, 8u);
    }
    return 0u;
}
...
void SferaDeflateState32::setDataType()
{
    ...
    while (symbol < 7u)
    {
        binary_frequency += this->dynamic_literal_tree[symbol++].code;
    }
    while (symbol < 128u)
    {
        ascii_frequency += this->dynamic_literal_tree[symbol++].code;
    }
    while (symbol < 256u)
    {
        binary_frequency += this->dynamic_literal_tree[symbol++].code;
...
uint32_t SferaDeflateState32::treeElementCount(SferaZlibTreeKind kind)
{
    switch (kind)
    {
    case SferaZlibTreeKind::Literal:
        return 286u;
    case SferaZlibTreeKind::Distance:
        return 30u;
    case SferaZlibTreeKind::BitLength:
        return 19u;
    default:
        return 0u;
    }
...
uint32_t SferaDeflateState32::treeMaxLength(SferaZlibTreeKind kind)
{
    return kind == SferaZlibTreeKind::BitLength ? 7u : 15u;
}

uint32_t SferaDeflateState32::treeExtraBits(SferaZlibTreeKind kind, uint32_t symbol)
{
    if (kind == SferaZlibTreeKind::Literal && symbol >= 257u)
    {
        const uint32_t code = symbol - 257u;
        return code >= 8u && code < 28u ? (code - 4u) >> 2u : 0u;
    }
    if (kind == SferaZlibTreeKind::Distance)
    {
        return symbol >= 4u && symbol < 30u ? (symbol >> 1u) - 1u : 0u;
    }
    if (kind == SferaZlibTreeKind::BitLength)
    {
        if (symbol == 16u)
        {
            return 2u;
...
{
    if (kind == SferaZlibTreeKind::Literal)
    {
        return fixedLiteralLength(symbol);
    }
    if (kind == SferaZlibTreeKind::Distance)
    {
        return 5u;
...
void SferaDeflateState32::pqDownHeap(SferaCtData32 *tree, uint32_t heap_index)
{
    ...
    {
        if (child < this->heap_length && heapLess(tree, this->heap[child + 1u], this->heap[child]))
        {
            ++child;
        }
        if (heapLess(tree, value, this->heap[child]))
        {
            break;
        }
...
void SferaDeflateState32::generateCodes(SferaCtData32 *tree, uint32_t symbol_count, const uint16_t *bit_counts)
{
    ...
    {
        code = (code + bit_counts[bits - 1u]) << 1u;
        next_code[bits] = code & 65535u;
    }
    for (uint32_t symbol = 0; symbol < symbol_count; ++symbol)
    {
        const uint32_t length = tree[symbol].length;
        if (length != 0u)
        {
            tree[symbol].code = reverseBits(next_code[length]++, length);
        }
    }
}
...
void SferaDeflateState32::generateBitLengths(SferaTreeDesc32 *descriptor)
{
    ...
    for (uint32_t bits = 0u; bits <= 15u; ++bits)
    {
        this->bit_length_counts[bits] = 0u;
    }
    ...
    for (uint32_t heap_index = this->heap_max + 1u; heap_index < 573u; ++heap_index)
    {
        ...
        if (symbol >= descriptor->symbol_count)
        {
            continue;
        }
        ...
        if (fixed_length != 0u)
        {
            this->static_length += frequency * (fixed_length + extra);
        }
    }
    while (overflow > 0)
    {
        uint32_t bits = max_length - 1u;
        while (bits > 0 && this->bit_length_counts[bits] == 0u)
        {
            --bits;
        }
        --this->bit_length_counts[bits];
    ...
    for (uint32_t bits = max_length; bits != 0; --bits)
    {
        ...
        while (remaining != 0u)
        {
            ...
            if (tree[symbol].length != bits)
            {
                this->optimal_length += (bits - tree[symbol].length) * tree[symbol].code;
                tree[symbol].length = static_cast<std::uint16_t>(bits);
            }
            --remaining;
        }
...
{
    for (uint32_t symbol = 0u; symbol != 286u; ++symbol)
    {
        this->dynamic_literal_tree[symbol].code = 0u;
    }
    for (uint32_t symbol = 0u; symbol != 30u; ++symbol)
    {
        this->dynamic_distance_tree[symbol].code = 0u;
    }
    for (uint32_t symbol = 0u; symbol != 19u; ++symbol)
    {
        this->bit_length_tree[symbol].code = 0u;
    }
...
void SferaDeflateState32::buildTree(SferaTreeDesc32 *descriptor)
{
    ...
    {
        if (tree[symbol].code != 0u)
        {
            this->heap[++this->heap_length] = symbol;
            symbol_count = symbol + 1u;
            this->depth[symbol] = 0u;
        }
        else
        {
            tree[symbol].length = 0u;
        }
    }
    while (this->heap_length < 2)
    {
        ...
        if (fixed_length != 0u)
        {
            this->static_length -= fixed_length;
        }
    }
    descriptor->symbol_count = symbol_count;
    for (uint32_t heap_index = this->heap_length / 2; heap_index >= 1; --heap_index)
    {
        pqDownHeap(tree, heap_index);
    }
...
void SferaDeflateState32::scanTree(SferaCtData32 *tree, uint32_t symbol_count)
{
    ...
    for (uint32_t symbol = 0; symbol < symbol_count; ++symbol)
    {
        ...
        ++count;
        if (count < maximum_count && current_length == next_length)
        {
            continue;
        }
        if (count < minimum_count)
        {
            this->bit_length_tree[current_length].code += static_cast<std::uint16_t>(count);
        }
        else if (current_length != 0)
        {
            if (current_length != previous_length)
            {
                ++this->bit_length_tree[current_length].code;
            }
            ++this->bit_length_tree[16].code;
        }
        else if (count <= 10u)
        {
            ++this->bit_length_tree[17].code;
        }
        else
        {
            ++this->bit_length_tree[18].code;
        }
        count = 0u;
...
void SferaDeflateState32::sendTree(SferaCtData32 *tree, uint32_t symbol_count)
{
    ...
    for (uint32_t symbol = 0; symbol < symbol_count; ++symbol)
    {
        ...
        if (count < minimum_count)
        {
            while (count-- != 0u)
            {
                sendTreeCode(bit_tree, current_length, false);
            }
        }
        else if (current_length != 0)
        {
        ...
        {
            sendTreeCode(bit_tree, 17u, false);
            sendBits(count - 3u, 3u);
        }
        else
        {
            sendTreeCode(bit_tree, 18u, false);
            sendBits(count - 11u, 7u);
        }
        count = 0u;
        previous_length = current_length;
        if (next_length == 0)
...
uint32_t SferaDeflateState32::buildBitLengthTree()
{
    ...
    while (last_rank >= 3u && bit_length_tree[SferaZlibTables::codeLengthOrder[last_rank]].length == 0u)
        --last_rank;
...
void SferaDeflateState32::sendAllTrees(uint32_t literal_codes, uint32_t distance_codes, uint32_t bit_length_codes)
{
    ...
    for (uint32_t rank = 0u; rank < bit_length_codes; ++rank)
        sendBits(bit_length_tree[SferaZlibTables::codeLengthOrder[rank]].length, 3u);
...
void SferaDeflateState32::flushBlock(const uint8_t *buffer, uint32_t stored_length, uint32_t end_of_file)
{
    ...
    if (this->level > 0)
    {
        ...
        if (static_bytes <= optimal_bytes)
        {
            optimal_bytes = static_bytes;
        }
    }
    else
    {
        optimal_bytes = static_bytes = stored_length + 5u;
    }
    if (buffer != nullptr && stored_length + 4u <= optimal_bytes)
    {
        writeStoredBlock(buffer, stored_length, end_of_file);
    }
    else if (static_bytes == optimal_bytes)
    {
        sendBits(2u + (end_of_file & 1u), 3u);
        compressBlock(nullptr, nullptr);
    }
    else
    {
...
void SferaDeflateState32::compressBlock(SferaCtData32 *literal_tree, SferaCtData32 *distance_tree)
{
    ...
    for (uint32_t index = 0u; index != literal_count; ++index)
    {
        ...
        if (distance == 0u)
        {
            sendTreeCode(literal_tree, literal_or_length, false);
            continue;
        }
        uint32_t length_code = literal_or_length;
        if (literal_or_length >= 255u)
        {
            length_code = 28u;
        }
        else if (literal_or_length >= 8u)
        {
            uint32_t highest_bit = 0u;
            for (uint32_t value = literal_or_length; value > 1u; value >>= 1u)
            {
                ++highest_bit;
            }
            const uint32_t extra = highest_bit - 2u;
            const uint32_t base = 1u << (extra + 2u);
            length_code = 4u * extra + 4u + ((literal_or_length - base) >> extra);
        }
        ...
        if (length_extra != 0u)
        {
            const uint32_t group_begin = 4u * length_extra + 4u;
            const uint32_t base = (1u << (length_extra + 2u)) + ((length_code - group_begin) << length_extra);
            sendBits(literal_or_length - base, length_extra);
        }
        --distance;
        ...
        {
            uint32_t highest_bit = 0u;
            for (uint32_t value = distance; value > 1u; value >>= 1u)
            {
                ++highest_bit;
            }
            const uint32_t extra = highest_bit - 1u;
            distance_code = (highest_bit << 1u) + ((distance >> extra) & 1u);
        }
        ...
        {
            const uint32_t base = (1u << (distance_extra + 1u)) + ((distance_code & 1u) << distance_extra);
            sendBits(distance - base, distance_extra);
        }
    }
...
bool SferaDeflateState32::tally(uint32_t distance, uint32_t literal_or_length)
{
    ...
    if (distance == 0u)
    {
        ++this->dynamic_literal_tree[literal_or_length].code;
    }
    else
    {
        ++this->matches;
        ...
        if (normalized_distance >= 4u)
        {
            uint32_t highest_bit = 0u;
            for (uint32_t value = normalized_distance; value > 1u; value >>= 1u)
            {
                ++highest_bit;
            }
            distance_code = (highest_bit << 1u) + ((normalized_distance >> (highest_bit - 1u)) & 1u);
        }
        uint32_t length_code = literal_or_length;
        if (literal_or_length >= 255u)
        {
        ...
        else if (literal_or_length >= 8u)
        {
            ...
            const uint32_t extra = highest_bit - 2u;
            length_code = 4u * extra + 4u + ((literal_or_length - (1u << (extra + 2u))) >> extra);
        }
...
std::optional<SferaDeflateBlockState> SferaDeflateState32::flushCurrentBlock(bool end_of_file)
{
    ...
    if (strm->avail_out == 0u)
        return end_of_file ? SferaDeflateBlockState::FinalNeedOutput : SferaDeflateBlockState::NeedOutput;
    return std::nullopt;
}
...
int SferaZStream32::deflateReset()
{
    ...
    state->phase = state->noheader != 0 ? SferaDeflatePhase::Blocks : SferaDeflatePhase::Header;
    this->adler = 1u;
    state->last_flush = 0;
    state->treeInit();
    state->deflateLmInit();
...
int SferaZStream32::deflate(int32_t flush)
{
    ...
    if (this->next_out == nullptr || (this->next_in == nullptr && this->avail_in != 0u) || (state->phase == SferaDeflatePhase::Complete && flush != 4))
    {
        this->msg = "stream error";
        return -2;
    }
    if (this->avail_out == 0u)
    {
        this->msg = "buffer error";
        return -5;
    }
    state->strm = this;
    const int32_t previous_flush = state->last_flush;
    state->last_flush = flush;
    if (state->phase == SferaDeflatePhase::Header)
    {
        ...
        if (state->strstart != 0u)
        {
            header |= preset_dictionary_flag;
        }
        header += 31u - header % 31u;
        state->phase = SferaDeflatePhase::Blocks;
    ...
    else if (this->avail_in == 0u && flush <= previous_flush && flush != 4)
    {
        this->msg = "buffer error";
        return -5;
    }
    if (state->phase == SferaDeflatePhase::Complete && this->avail_in != 0u)
    {
        this->msg = "buffer error";
        return -5;
    }
    if (this->avail_in != 0u || state->lookahead != 0u || (flush != 0 && state->phase != SferaDeflatePhase::Complete))
    {
        SferaDeflateBlockState block_state;
        if (state->level == 0)
        {
            block_state = state->deflateStored(flush);
        }
        else if (state->level <= 3)
        {
            block_state = state->deflateFast(flush);
        }
        else
        {
            block_state = state->deflateSlow(flush);
        }
        if (block_state == SferaDeflateBlockState::FinalNeedOutput || block_state == SferaDeflateBlockState::Complete)
        {
            state->phase = SferaDeflatePhase::Complete;
        }
        if (block_state == SferaDeflateBlockState::NeedOutput || block_state == SferaDeflateBlockState::FinalNeedOutput)
        {
            if (this->avail_out == 0u)
            {
                state->last_flush = -1;
        ...
        if (block_state == SferaDeflateBlockState::BlockDone)
        {
            if (flush == 1)
            {
                state->alignStaticBlock();
            }
            else
            {
                ...
                if (flush == 3)
                {
                    std::fill(state->head.begin(), state->head.end(), std::uint16_t{});
                }
            }
            deflateFlushPending();
            if (this->avail_out == 0u)
...
SferaDeflateBlockState SferaDeflateState32::deflateStored(int32_t flush)
{
    uint32_t max_block_size = std::numeric_limits<uint16_t>::max();
    if (max_block_size > this->pending_buf_size - 5u)
    {
        max_block_size = this->pending_buf_size - 5u;
    }
    for (;;)
    {
        ...
        {
            deflateFillWindow();
            if (this->lookahead == 0u && flush == 0)
            {
                return SferaDeflateBlockState::NeedOutput;
            }
            if (this->lookahead == 0u)
            {
                break;
            }
        }
    ...
    return flushCurrentBlock(finishing).value_or(finishing ? SferaDeflateBlockState::Complete : SferaDeflateBlockState::BlockDone);
}

SferaDeflateBlockState SferaDeflateState32::deflateFast(int32_t flush)
{
    ...
    for (;;)
    {
        ...
        {
            deflateFillWindow();
            if (this->lookahead < 262u && flush == 0)
            {
                return SferaDeflateBlockState::NeedOutput;
            }
            if (this->lookahead == 0u)
        ...
        {
            hash_head = insertString(window_bytes, previous, heads);
        }
        if (hash_head != 0u && this->strstart - hash_head <= this->w_size - 262u && this->strategy != 2)
        {
...
SferaDeflateBlockState SferaDeflateState32::deflateSlow(int32_t flush)
{
    uint8_t *window_bytes = this->window.data();
    uint16_t *previous = this->prev.data();
    uint16_t *heads = this->head.data();
    for (;;)
    {
        ...
        {
            this->match_length = deflateLongestMatch(hash_head);
            if (this->match_length <= 5u && (this->strategy == 1 || (this->match_length == 3u && this->strstart - this->match_start > 4096u)))
            {
                this->match_length = 2u;
            }
        }
    ...
    {
        tally(0u, window_bytes[this->strstart - 1u]);
        this->match_available = 0;
    }
    const bool finishing = flush == 4;
    return flushCurrentBlock(finishing).value_or(finishing ? SferaDeflateBlockState::Complete : SferaDeflateBlockState::BlockDone);
...
uint32_t SferaZStream32::deflateReadBuf(uint8_t *destination, uint32_t size)
{
    ...
    if (state->noheader == 0)
    {
        this->adler = adler32(this->adler, this->next_in, length);
    }
...
uint32_t SferaDeflateState32::deflateLongestMatch(uint32_t current_match)
{
    ...
    if (nice_length > this->lookahead)
    {
        nice_length = this->lookahead;
    }
    ...
    if (best_length >= compare_limit || chain_left == 0u)
    {
        return best_length < this->lookahead ? best_length : this->lookahead;
    }
    while (true)
    {
        if (current_match >= this->strstart)
        {
            break;
        }
        if (window_bytes[current_match] == window_bytes[this->strstart] && window_bytes[current_match + 1u] == window_bytes[this->strstart + 1u] &&
            window_bytes[current_match + best_length] == window_bytes[this->strstart + best_length])
        {
            uint32_t length = 2u;
            while (length < compare_limit && window_bytes[current_match + length] == window_bytes[this->strstart + length])
            {
                ++length;
            }
...
void SferaDeflateState32::deflateFillWindow()
{
    ...
    while (true)
    {
        ...
        if (more == 0u && this->strstart == 0u && this->lookahead == 0u)
        {
            more = window_half_size;
        }
        else if (more == UINT32_MAX)
        {
            --more;
        }
        else if (this->strstart >= window_half_size + (window_half_size - 262u))
        {
            ...
            for (uint32_t index = 0u; index != this->hash_size; ++index)
            {
                const uint32_t value = heads[index];
                heads[index] = static_cast<std::uint16_t>(value >= window_half_size ? value - window_half_size : 0u);
            }
            for (uint32_t index = 0u; index != window_half_size; ++index)
            {
                const uint32_t value = previous[index];
                previous[index] = static_cast<std::uint16_t>(value >= window_half_size ? value - window_half_size : 0u);
            }
            more += window_half_size;
        }
        if (this->strm->avail_in == 0u)
        {
            return;
        }
        ...
        if (this->lookahead >= 262u || this->strm->avail_in == 0u)
        {
            return;
        }
    }
...
int SferaZStream32::compress(std::uint8_t *output, std::uint32_t &outputSize, std::span<const std::uint8_t> input, int level)
{
    if (level == -1)
        level = 6;
    if (level < 0 || level > 9 || input.size() > std::numeric_limits<std::uint32_t>::max() || output == nullptr)
        return -2;
    try
    {
...
bool SferaInflateHuft32::build(std::span<const std::uint8_t> lengths, bool codeLengths)
{
    ...
    if (unused != 0 && (codeLengths || maximum_length != 1))
        return false;
    std::array<std::uint16_t, 16> offsets{};
    for (std::size_t bits = 1; bits + 1 < offsets.size(); ++bits)
...
int SferaZStream32::inflate()
{
    ...
    if (!readBits(8, methodByte) || !readBits(8, flags))
        return -5;
    const auto method = methodByte & 15u;
    const auto windowBits = (methodByte >> 4u) + 8u;
    if (method != 8 || windowBits > 15 || (methodByte * 256u + flags) % 31u != 0)
        return -3;
    ...
    do
    {
        ...
        if (!readBits(1, finalBlock) || !readBits(2, blockType))
            return -5;
        if (blockType == 0)
        {
            ...
            if (!readBits(16, length) || !readBits(16, complement))
                return -5;
            if ((length ^ complement) != std::numeric_limits<std::uint16_t>::max())
                return -3;
            for (std::uint32_t i = 0; i < length; ++i)
            {
                std::uint32_t value;
                if (avail_out == 0 || !readBits(8, value))
                    return -5;
        ...
        if (blockType == 1)
        {
            for (std::uint32_t symbol = 0; symbol < literalCount; ++symbol)
                lengths[symbol] = SferaDeflateState32::fixedLiteralLength(symbol);
            std::fill(lengths.begin() + literalCount, lengths.end(), std::uint8_t{5});
        }
        else
        {
            std::uint32_t codeCount;
            if (!readBits(5, literalCount) || !readBits(5, distanceCount) || !readBits(4, codeCount))
                return -5;
            literalCount += 257;
            distanceCount += 1;
            codeCount += 4;
            ...
            {
                std::uint32_t length;
                if (!readBits(3, length))
                    return -5;
                codeLengths[SferaZlibTables::codeLengthOrder[i]] = static_cast<std::uint8_t>(length);
            }
            SferaInflateHuft32 codeTree;
            if (!codeTree.build(codeLengths, true))
                return -3;
            std::uint32_t index = 0;
        ...
        if (lengths[256] == 0 || !literalTree.build(std::span(lengths).first(literalCount)) || !distanceTree.build(std::span(lengths).subspan(literalCount, distanceCount)))
            return -3;
        for (;;)
        {
            ...
            if (!readBits(lengthBits, lengthExtra))
                return -5;
            ...
            if (!readBits(distanceBits, distanceExtra))
                return -5;
            ...
            if (distance > total_out || distance > (1u << windowBits))
                return -3;
            if (length > avail_out)
                return -5;
            for (std::uint32_t i = 0; i < length; ++i)
            {
                *next_out = *(next_out - distance);
                ++next_out;
            }
            avail_out -= length;
            total_out += length;
...
{
    if (output == nullptr || input.size() > std::numeric_limits<std::uint32_t>::max())
        return -2;
    SferaZStream32 stream;
    stream.next_in = input.data();
    stream.avail_in = static_cast<std::uint32_t>(input.size());
...
{
    if (output == nullptr || outputSize < envelope_header_size || input.size() > std::numeric_limits<std::uint32_t>::max())
        return -1;
    std::uint32_t payloadSize = outputSize - envelope_header_size;
    auto *payload = output + envelope_header_size;
    if (compress(payload, payloadSize, input, 1) != 0)
        return -1;
    if (payloadSize <= envelope_length_mask_index)
        return -1;
    ...
    for (std::size_t i = 0; i < sizeof(std::uint32_t); ++i)
        output[4 + i] = ((originalSize >> (8u * i)) & 255u) ^ payload[envelope_length_mask_index];
...
{
    if (!hasEnvelope(input) || input.size() <= envelope_header_size + envelope_length_mask_index)
        return -2;
    ...
    for (std::size_t i = 0; i < sizeof(outputSize); ++i)
        input[4 + i] ^= payload[envelope_length_mask_index];
    return decompress(output, outputSize, payload) == 0 ? 0 : -1;
}

void SferaZStream32::appendUiByte(std::vector<std::uint8_t> &output, std::size_t limit, std::uint8_t value)
{
