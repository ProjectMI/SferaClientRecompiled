...
namespace {
    ...
    constexpr int maximumScrollOffset = INT32_MAX;
...
namespace SphereUI::detail {
    ...
    std::uint32_t wireWord(Integer value) {
        static_assert(std::is_integral_v<Integer>);
        if constexpr (std::is_signed_v<Integer>) {
            if (!std::in_range<std::int32_t>(value)) throw std::overflow_error("UI value exceeds 32-bit wire range");
    ...
    std::int32_t signedWord(std::uint32_t value) noexcept { return SferaNumeric::signedWord(value); }

    template<class Integer>
    int checkedInt(Integer value, std::string_view context) {
        static_assert(std::is_integral_v<Integer>);
        if (!std::in_range<int>(value)) throw std::overflow_error(std::string(context));
        return SferaNumeric::signedWord(SferaNumeric::lowWord(value));
    }

    template<class Integer>
        requires std::is_integral_v<Integer>
    double real64(Integer value) noexcept {
        return static_cast<double>(value);
    }

    template<class Left, class Right>
    int multiplyCoordinate(Left left, Right right) {
        static_assert(std::is_integral_v<Left> && std::is_integral_v<Right>);
        const auto product = std::int64_t{checkedInt(left, "UI coordinate factor exceeds int")} * checkedInt(right, "UI coordinate factor exceeds int");
        return checkedInt(product, "UI coordinate product exceeds int");
    }

    template<class Left, class Right>
    int addCoordinate(Left left, Right right) {
        static_assert(std::is_integral_v<Left> && std::is_integral_v<Right>);
        const auto sum = wireWord(left) + wireWord(right);
        return signedWord(sum);
    }

    template<class Left, class Right>
    int subtractCoordinate(Left left, Right right) {
        static_assert(std::is_integral_v<Left> && std::is_integral_v<Right>);
        const auto difference = wireWord(left) - wireWord(right);
        return signedWord(difference);
    }
...
struct SphereUI::Window::CloneContext {
    ...
    template<class T> requires std::is_same_v<std::remove_const_t<T>, Window>
    T* resolve(T* source) const {
        if (source == nullptr) return nullptr;
        const auto found = windows.find(source);
        if (found != windows.end()) return found->second;
        return g_sfera_interface.isRegistered(source) ? source : nullptr;
    }
...
void SphereUI::Window::initializeCopy(const Window& source, CloneContext& context) {
    ...
    if (source.tooltip) {
        ...
        if (typed == nullptr) throw std::logic_error("UI tooltip clone changed type");
        copy.release();
        tooltip.reset(typed);
    } else tooltip.reset();
...
double SphereUI::Runtime::elapsedSeconds(std::uint64_t started) {
    const auto raw = clockTicks() - started;
    std::int64_t ticks{};
    std::memcpy(&ticks, &raw, sizeof(ticks));
    const double value = detail::real64(ticks);
...
bool SphereUI::Runtime::playerHeading(float& heading) {
    ...
    if (SferaNumeric::signedWord(handle) <= 0) return false;
...
void SphereUI::Runtime::clipboardText(std::string& result) {
    ...
    if (end != bytes.end()) result.assign(SferaText::fromBytes(std::span(bytes.begin(), end)));
}
...
namespace {
    ...
    public:
        explicit ViewportScope(const SphereUI::UiRect& bounds, bool restore_viewport = true) : restore(restore_viewport) {
            ...
            SphereUI::InterfaceRenderer::setViewport({left, top, right - left, bottom - top, 0.0f, 1.0f});
            changed = true;
        }
    ...
    public:
        ...
        template<class T> void integer(std::string_view key, T& value) {
            ...
            if (!std::in_range<T>(parsed)) throw std::out_of_range("UI integer value is outside target range");
            value = parsed;
        }
        ...
        void boolean(std::string_view key, bool& value) {
            if (has(key)) value = parser.readBool(0u);
        }
    ...
    void notifyParent(SphereUI::Window& window, SphereUI::UiMessage message, std::size_t first = 0u, std::size_t second = 0u) {
        if (auto* owner = window.parent) {
            owner->queueEvent({&window, window.control_id, message, SphereUI::detail::wireWord(first), SphereUI::detail::wireWord(second)});
        }
    }

    template<class T> T* typedControl(SphereUI::Window* control) {
        if (control == nullptr) return nullptr;
        if constexpr (std::is_same_v<T, SphereUI::Window>) return control;
        else if constexpr (std::is_same_v<T, SphereUI::ButtonCtrl>) return control->asButtonCtrl();
        else if constexpr (std::is_same_v<T, SphereUI::ScrollBar>) return control->asScrollBar();
        else if constexpr (std::is_same_v<T, SphereUI::SpinButton>) return control->asSpinButton();
        else static_assert(std::is_same_v<T, void>, "Unsupported UI control ownership type");
    }
    ...
    template<class T> void cloneOwned(std::unique_ptr<T>& destination, const std::unique_ptr<T>& source, SphereUI::Window& owner, SphereUI::Window::CloneContext& context) {
        ...
        auto* typed = typedControl<T>(copy.get());
        if (copy && typed == nullptr) throw std::logic_error("UI clone changed control type");
        if (typed) typed->setParent(&owner);
        copy.release();
    ...
    template<class T> bool loadPart(SphereUI::Window& owner, std::unique_ptr<T>& destination, UiReader& reader, std::string_view key, const std::string& filename, SferaSimpleParser& parser, UiControlKind kind, std::uint32_t id) {
        ...
        auto* typed = typedControl<T>(control.get());
        if (control && typed == nullptr) throw std::logic_error("UI factory returned unexpected control type");
        if (typed) typed->setParent(&owner);
        control.release();
        destination.reset(typed);
    ...
    template<class T> void loadNavigationButtons(T& owner, UiReader& reader, const std::string& filename, SferaSimpleParser& parser, bool defaults, float interval = 0.0f) {
        ...
        for (std::size_t index = 0u; index < 2u; ++index) if (loadPart(owner, *destinations[index], reader, names[index], filename, parser, UiControlKind::button, SphereUI::detail::wireWord(index + 1u)) && defaults) {
...
bool SphereUI::Window::loadUi(const std::string& filename, SferaSimpleParser& parser, const SferaParserRange& range) {
    ...
    while (parser.nextBlock("control", &entry)) {
        ...
        if (class_name) if (auto child = createControl(filename, parser, entry, detail::controlKind(*class_name), ordinal)) {
...
std::uint32_t SphereUI::Window::handleMessage(SphereUI::UiMessage message, std::uint32_t first, std::uint32_t second) {
    ...
    switch (message) {
        ...
        case UiMessage::setPosition:
            setPosition(detail::signedWord(first), detail::signedWord(second));
            break;
...
void SphereUI::Window::draw() {
    ...
    if ((behavior_flags & WindowStyle::skipDrawing) == 0u && resource_reference != nullptr) resource_reference->drawNatural(SferaNumeric::real32(screen_x), SferaNumeric::real32(screen_y), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
    if ((behavior_flags & WindowStyle::showTitle) != 0u && text.size() != 0u) {
        ...
        if (SferaNumeric::word(measured.width) > SferaNumeric::word(caption_width)) {
            ...
            top = detail::addCoordinate(top, SferaNumeric::signedWord((SferaNumeric::word(measured.height) - SferaNumeric::word(smaller.height)) >> 1u));
        }
...
bool SphereUI::ButtonCtrl::loadUi(const std::string&, SferaSimpleParser& parser, const SferaParserRange& range) {
    ...
    if (const auto hotkey_name = reader.string("hotKey", true)) {
        hotkey = Runtime::keyCode(*hotkey_name);
        ...
        for (std::size_t index = 0u; index < 3u; ++index) {
            ...
            if (modifier == 1 || modifier == 2) hotkey |= modifier << (16u + 2u * index);
        }
    }
...
std::unique_ptr<SphereUI::Window> SphereUI::ButtonCtrl::cloneInto(CloneContext& context) const {
    return cloneControl(*this, context, [](auto& target, const auto& source, SphereUI::Window::CloneContext&) {
...
void SphereUI::ButtonCtrl::draw() {
    ...
    if ((button_flags & ButtonStyle::stateImages) != 0u) {
        ...
        if (image != nullptr) image->draw(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top), SferaNumeric::real32(bounds.right), SferaNumeric::real32(bounds.bottom), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
    }
...
void SphereUI::ButtonCtrl::handleInput(const WindowInput& input) {
    ...
    if (containsPoint(input.mouse_x, input.mouse_y)) {
        ...
        if ((button_flags & ButtonStyle::repeatWhilePressed) != 0u && pressed && (input.mouse_buttons & 1u) != 0u) {
            const float elapsed = SferaNumeric::real32(Runtime::elapsedSeconds(repeat_started_at));
...
void SphereUI::CheckBox::draw() {
    ...
    if (image != nullptr) image->drawNatural(SferaNumeric::real32(detail::addCoordinate(bounds.left, image_x)), SferaNumeric::real32(detail::addCoordinate(bounds.top, image_y)), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
    int dx = label_x;
    int dy = label_y;
...
void SphereUI::CDescriptionWindow::draw() {
    ...
    InterfaceRenderer::drawTexture(nullptr, left_x - 0.5f + left->width, top_y - 0.5f + caption->height, SferaNumeric::real32(bounds.right) - 0.5f - right->width, SferaNumeric::real32(bounds.bottom) - 0.5f - lower->height, SferaColor::fromArgb(UiColor::frameBackground).withAlpha(alpha * 166u / 255u).argb(), 0.0f, 0.0f, false);
...
std::unique_ptr<SphereUI::Window> SphereUI::EditCtrl::cloneInto(CloneContext& context) const {
    return cloneControl(*this, context, [](EditCtrl& target, const EditCtrl& source, SphereUI::Window::CloneContext&) {
    ...
    if (input_enabled) {
        ...
        switch (input.key_code) {
            case VK_BACK:
                ...
                break;
            case VK_DELETE:
                if (caret_position < value.size()) value.erase(caret_position, 1u);
                break;
            case VK_LEFT:
                if (caret_position > 0) --caret_position;
                break;
            case VK_RIGHT:
                if (caret_position < value.size()) ++caret_position;
                break;
            case VK_HOME:
                caret_position = 0;
                break;
...
void SphereUI::EditCtrl::draw() {
    ...
    if (full_width > width) {
        dx = width - full_width;
        if (caret_x < -1LL * dx) dx = caret_position == 0 ? 0 : cursor_width - caret_x;
    } else {
    ...
    if (elapsed > 4000u) cursor_visible = (((elapsed - 4000u) / 4000u) & 1u) != 0u;
...
std::uint32_t SphereUI::ListCtrl::handleMessage(SphereUI::UiMessage message, std::uint32_t first, std::uint32_t second) {
    switch (message) {
        ...
        case UiMessage::setListSelection:
            selectRow(detail::signedWord(first));
            return 1u;
...
void SphereUI::ListCtrl::draw() {
    ...
    if (rows.size() != 0u) {
        ...
        if (viewport) {
            ...
            if (chatlike && std::ssize(rows) < std::max(visible_capacity, std::ptrdiff_t{})) row_y = detail::addCoordinate(row_y, detail::subtractCoordinate(height, detail::multiplyCoordinate(rows.size(), line_height)));
            ...
            if (selected_index >= visible_begin && selected_index < end) drawSelection(bounds.left, bounds.top, detail::addCoordinate(first_y, detail::multiplyCoordinate(selected_index - visible_begin, line_height)), false);
        }
    }
...
void SphereUI::ListCtrl::handleInput(const WindowInput& input) {
    ...
    if (chatlike && std::ssize(rows) < std::max(visible_capacity, std::ptrdiff_t{})) first_y = detail::addCoordinate(first_y, detail::subtractCoordinate(height, detail::multiplyCoordinate(rows.size(), line_height)));
    ...
    if (selected != selected_index) {
        ...
        notifyParent(*this, UiMessage::listSelectionChanged, detail::wireWord(selected));
    }
    const auto now = Runtime::clockTicks();
    if (!click_pending) {
        click_pending = true;
        ...
        const double elapsed = detail::real64(SferaNumeric::signedWord(now - click_time));
        const auto seconds = elapsed / 10000.0;
        if (seconds < 0.6 && selected_index >= 0) notifyParent(*this, UiMessage::listDoubleClick, detail::wireWord(selected_index));
        click_pending = false;
    }
}
...
void SphereUI::FontPicker::setFont(int font_id) {
    if (auto* spin = selector.get()) spin->setCurrentValue(font_id - 2);
...
void SphereUI::ImageCtrl::draw() {
    ...
    const auto bounds = windowBounds(*this);
    resource_reference->draw(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top), SferaNumeric::real32(bounds.right), SferaNumeric::real32(bounds.bottom), SferaColor::rgba(255u, 255u, 255u, alpha).argb(), rotated ? rotation_radians : 0.0f);
}
...
void SphereUI::ImageCtrl::handleInput(const WindowInput& input) {
    ...
    if (inside && (image_style & 1u) != 0u && (input.mouse_flags & MouseInput::leftPress) != 0u) {
        ...
        notifyParent(*this, UiMessage::leftClick, input.mouse_x - left, input.mouse_y - top);
        Runtime::playClickSound();
    }
}

void SphereUI::ImageCtrl::setOpacity(float new_opacity) {
    const std::uint32_t new_alpha = opacity_alpha(base_alpha * new_opacity);
    alpha = new_alpha;
    replace_color_alpha(text_color, new_alpha);
    replace_color_alpha(disabled_color, new_alpha);
    opacity = new_opacity;
...
bool SphereUI::ListItemCtrl::loadUi(const std::string& filename, SferaSimpleParser& parser, const SferaParserRange& range) {
    ...
    if (selected_index < 0 || selected_index >= std::ssize(items)) selected_index = -1;
...
void SphereUI::ListItemCtrl::draw() {
    ...
    const auto end = std::min(visible_end, std::ssize(items));
    {
        ...
        if (!viewport) return;
        if (selected_index >= std::max(visible_begin, std::ptrdiff_t{}) && selected_index < end) if (auto* item = itemAt(selected_index)) {
            const float left = SferaNumeric::real32(item->x), top = SferaNumeric::real32(item->y);
            const float right = SferaNumeric::real32(detail::addCoordinate(item->x, item->width));
            const float bottom = SferaNumeric::real32(detail::addCoordinate(item->y, item->height));
        ...
        for (auto index = std::max(visible_begin, std::ptrdiff_t{}); index < end; ++index) if (auto* item = itemAt(index)) item->draw();
    }
    for (auto index = std::max(visible_begin, std::ptrdiff_t{}); index < end; ++index) if (auto* item = itemAt(index)) item->drawToolTips();
}
...
void SphereUI::ListItemCtrl::handleInput(const WindowInput& input) {
    ...
    for (std::size_t index = 0u; index < items.size(); ++index) if (auto* item = itemAt(index)) {
        WindowEvent event{};
        while (item->pollEvent(event)) if (auto* owner = parent) owner->queueEvent({this, control_id, UiMessage::listItemEvent, SferaNumeric::enumBits(event.message), SferaNumeric::word(MAKELONG(index, event.control_id))});
    }
    const auto bounds = windowBounds(*this);
    ...
    for (auto index = std::max(visible_begin, std::ptrdiff_t{}); index < end; ++index) if (auto* item = itemAt(index)) {
    ...
    for (auto index = std::max(visible_begin, std::ptrdiff_t{}); index < end; ++index) if (auto* item = itemAt(index)) if (contains(windowBounds(*item), input.mouse_x, input.mouse_y)) {
        selected = index;
        break;
    }
    if (selected != selected_index) {
        notifyParent(*this, UiMessage::listItemSelectionChanged, detail::wireWord(selected));
        selected_index = selected;
    }
}
...
bool SphereUI::ListItemCtrl::hitTest(int screen_x, int screen_y) {
    ...
    for (auto index = std::max(visible_begin, std::ptrdiff_t{}); index < end; ++index) if (auto* child = itemAt(index)) if (child->can_drag_drop && child->containsPoint(screen_x, screen_y) && child->hitTest(screen_x, screen_y)) break;
    return true;
}
...
bool SphereUI::ToolTipCtrl::loadUi(const std::string&, SferaSimpleParser& parser, const SferaParserRange& range) {
    UiReader reader(parser, range);
...
std::unique_ptr<SphereUI::Window> SphereUI::ToolTipCtrl::cloneInto(CloneContext& context) const {
    return cloneControl(*this, context, [](ToolTipCtrl& destination, const ToolTipCtrl& source, SphereUI::Window::CloneContext&) {
...
void SphereUI::ToolTipCtrl::handleInput(const WindowInput& input) {
    ...
    if (hidden && !dismissed) {
        ...
        else if (Runtime::elapsedSeconds(hover_started) > 0.5f) showAt(input.mouse_x, input.mouse_y);
    }
...
void SphereUI::CMinimapControl::draw() {
    ...
    if (Runtime::playerHeading(heading) && resource_reference != nullptr) {
        ...
        const float arrow_x = SferaNumeric::real32(detail::addCoordinate(parent_x, 127));
        const float arrow_y = SferaNumeric::real32(detail::addCoordinate(parent_y, 132));
...
namespace {
    void updateStatusText(std::string& text, std::uint32_t mode, double percent, int current, int range) {
        if (mode == 1u) text = std::to_string(SferaNumeric::truncateInt(percent)) + "%";
...
std::uint32_t SphereUI::ProgressBar::handleMessage(SphereUI::UiMessage message, std::uint32_t first, std::uint32_t second) {
    ...
    switch (message) {
        case UiMessage::setProgressValue:
            setProgressValue(detail::signedWord(first));
            return 1u;
        case UiMessage::setProgressRange:
            setProgressRange(detail::signedWord(first), detail::signedWord(second));
            return 1u;
        default:
            return 0u;
    }
...
void SphereUI::ProgressBar::draw() {
    if (hidden || (behavior_flags & WindowStyle::skipDrawing) != 0u) return;
    const auto bounds = windowBounds(*this);
    if (progress_ratio > 0.0f && resource_reference != nullptr) resource_reference->draw(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top), bounds.left + width * progress_ratio, SferaNumeric::real32(bounds.bottom), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
    if (display_mode != 0u) {
...
std::uint32_t SphereUI::ScrollBar::handleMessage(SphereUI::UiMessage message, std::uint32_t first, std::uint32_t second) {
    switch (message) {
        case UiMessage::setScrollValue:
            current = std::min(std::max(detail::signedWord(first), minimum), maximum);
            updateControlState();
            return 1u;
...
void SphereUI::ScrollBar::draw() {
    ...
    const auto bounds = windowBounds(*this);
    const auto color = SferaColor::rgba(255u, 255u, 255u, alpha).argb();
    if (resource_reference != nullptr) resource_reference->draw(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top), SferaNumeric::real32(bounds.right), SferaNumeric::real32(bounds.bottom), color);
    if (maximum > minimum) {
        const auto left = detail::addCoordinate(bounds.left, thumb_x);
        const auto top = detail::addCoordinate(bounds.top, thumb_y);
        if (scroll_resource != nullptr) scroll_resource->draw(SferaNumeric::real32(left), SferaNumeric::real32(top), SferaNumeric::real32(detail::addCoordinate(left, thumb_width)), SferaNumeric::real32(detail::addCoordinate(top, thumb_height)), color);
    }
...
void SphereUI::ScrollBar::handleInput(const WindowInput& input) {
    ...
    if (input.wheel_delta != 0) if (auto* owner = parent) if (contains(windowBounds(*owner), input.mouse_x, input.mouse_y)) {
        ...
        current = detail::signedWord(detail::wireWord(current) - detail::wireWord(step) * detail::wireWord(input.wheel_delta));
    ...
    const int dimension = horizontal ? detail::subtractCoordinate(track.right, track.left) : detail::subtractCoordinate(track.bottom, track.top);
...
void SphereUI::ScrollBar::updateControlState() {
    ...
    thumb_x = detail::addCoordinate(track_left, (orientation_flags & 1u) != 0u ? SferaNumeric::truncateInt(horizontal_distance * normalized_position) : horizontal_space / 2);
    thumb_y = detail::addCoordinate(track_top, (orientation_flags & 1u) != 0u ? vertical_space / 2 : SferaNumeric::truncateInt(vertical_distance * normalized_position));
}
...
void SphereUI::ScrollBar::loadControlParameters() {
    ...
    owner->handleMessage(message, detail::wireWord(current), control_id);
    if (notify_changes) owner->queueEvent({this, control_id, message, detail::wireWord(current), 0u});
}
...
void SphereUI::SliderCtrl::loadControlParameters() {
    if (auto* owner = parent) owner->queueEvent({this, control_id, UiMessage::sliderValueChanged, detail::wireWord(current), 0u});
}
...
std::uint32_t SphereUI::SpinButton::handleMessage(SphereUI::UiMessage message, std::uint32_t first, std::uint32_t second) {
    switch (message) {
        case UiMessage::setSpinValue:
            setCurrentValue(detail::signedWord(first));
            return 1u;
        case UiMessage::setSpinRange:
            setRange(detail::signedWord(first), detail::signedWord(second));
            return 1u;
        case UiMessage::updateSpinStatus:
            updateStatus();
            return 1u;
        case UiMessage::setSpinStep:
            setStep(detail::signedWord(first));
            return 1u;
        default:
            break;
    }
...
void SphereUI::SpinButton::updateStatus() {
    ...
    if (current != previous) {
        ...
        if (notify_changes && update_enabled) owner->queueEvent({this, control_id, UiMessage::spinValueChanged, detail::wireWord(current), 0u});
        previous = current;
    }
    status_valid = true;
    update_enabled = true;
...
namespace {
    void drawChild(SphereUI::Window* address, std::uint32_t alpha) {
        if (auto* child = address) {
            if (child->alpha != alpha) child->setOpacity(alpha / 255.0f);
            child->draw();
        }
    }
...
void SphereUI::ImageCtrl::setImageName(std::string_view image_name) {
...
void SphereUI::ToolTipCtrl::showAt(int cursor_x, int cursor_y) {
    if (fade_opacity == 0.0f) fade_started = Runtime::clockTicks();
    hidden = false;
    screen_x = detail::addCoordinate(cursor_x, 10);
    screen_y = detail::addCoordinate(cursor_y, 20);
}
...
void SphereUI::ToolTipCtrl::appendLine(std::string_view line_text) {
    lines.emplace_back(line_text);
    updateLayout();
}

void SphereUI::ToolTipCtrl::setLine(std::uint32_t index, std::optional<std::string_view> line_text) {
    if (index >= lines.size()) {
        if (line_text) appendLine(*line_text);
        return;
    }
...
void SphereUI::CDescriptionWindow::showDescription(std::string_view description_text, const Window* source, std::uint32_t duration, bool pin) {
    Window* content = nullptr;
    detail::forEachChild(*this, [&](Window& child) {
        if (content == nullptr) content = &child;
    });
...
void SphereUI::CDescriptionWindow::requestDescription(std::string_view description_text, bool force, const Window* source) {
    if (description_text.empty()) return;
    if (force) {
    ...
    else if (WorldClock::deadlineReached(Runtime::milliseconds(), show_deadline)) showDescription(description_text, pending_source, 250u, false);
}
...
void SphereUI::ListItemCtrl::addItem() {
...
void SphereUI::ListItemCtrl::removeItem(std::size_t index) {
    ...
    if (changed && can_select) notifyParent(*this, UiMessage::listItemSelectionChanged, detail::wireWord(selected_index));
}
...
void SphereUI::ListItemCtrl::updateLayout() {
    ...
    const auto total_width = horizontal ? detail::multiplyCoordinate(items.size(), original->width) : original->width;
    const auto total_height = !horizontal ? detail::multiplyCoordinate(items.size(), original->height) : original->height;
    ...
    if (auto* bar = vertical_scroll.get()) bar->setParameters({0u, ScrollField::all, 0, maximum_y, height, vertical_offset, original->height / 10 + 1});
    if (auto* bar = horizontal_scroll.get()) bar->setParameters({0u, ScrollField::all, 0, maximum_x, width, horizontal_offset, maximum_x / 10 + 1});
    updateVisibleRange();
}
...
std::size_t SphereUI::ListCtrl::physicalIndex(std::size_t logical_index) const {
    ...
    if (maximum_items == 0u) return logical_index;
...
void SphereUI::ListCtrl::updateLayout() {
    ...
    const int extent = detail::multiplyCoordinate(rows.size(), line_height);
...
void SphereUI::ListCtrl::appendLine(std::string_view line_text, std::uint32_t color) {
    ...
    if (text_alignment != 0u) {
        const auto extent = InterfaceRenderer::measureText(line_text, font, font_initialized);
        ...
        if ((text_alignment & TextAlignment::horizontalCenter) != 0u) offset = detail::checkedInt(std::abs(1LL * remainder) / 2, "text offset exceeds int");
    }
    appendCircular(rows, maximum_items, write_index, UiTextRow{std::string(line_text), color, offset});
    updateLayout();
}

void SphereUI::ListCtrl::appendFormattedLine(std::string_view line_text, std::uint32_t color) {
    wrapTextLine(*this, line_text, format_strings, [&](std::string_view line) {
        appendLine(line, color);
    });
}

void SphereUI::ListCtrl::addText(std::string_view line_text, std::uint32_t color) {
    std::size_t begin = 0u;
    for (;;) {
...
void SphereUI::ListCtrl::appendMessageText(std::string_view line_text, std::uint32_t color) {
...
void SphereUI::ListCtrl::selectRow(int index) {
    if (index >= 0 && index < rows.size()) selected_index = index;
}
...
void SphereUI::ListCtrl::alignRow(std::size_t index, std::uint32_t flags) {
    ...
    if (flags == 0u) row.offset = 0;
        ...
        const auto remainder = detail::subtractCoordinate(width, extent.width);
        if ((flags & 4u) != 0u) row.offset = remainder / 2u;
...
void SphereUI::ListCtrl::drawSelection(int screen_x, int screen_y, int row_y, bool restore_viewport) {
    ...
    const float bottom = SferaNumeric::real32(detail::addCoordinate(row_y, line_height));
...
void SphereUI::FilterListCtrl::appendHistory(std::string_view history_text, std::uint32_t color, std::uint32_t mask) {
    appendCircular(history, maximum_items, history_write, HistoryEntry{std::string(history_text), color, mask});
}

void SphereUI::FilterListCtrl::appendFilteredText(std::string_view filtered_text, std::uint32_t packed_color) {
    ...
    if ((filter_mask & mask) != 0u) appendMessageText(filtered_text, color);
}
...
std::unique_ptr<SphereUI::Window> SphereUI::CMenuListControl::cloneInto(CloneContext& context) const {
    return cloneControl(*this, context, [](auto& destination, const auto& source, SphereUI::Window::CloneContext&) {
...
void SphereUI::CMenuListControl::keepOnScreen() {
    ...
    if (next_x != parent_screen_x || next_y != parent_screen_y) {
        owner->handleMessage(UiMessage::setPosition, detail::wireWord(next_x), detail::wireWord(next_y));
        updateParentPosition();
    }
}

void SphereUI::CMenuListControl::addItem(std::string_view item_text, bool enabled) {
    if (items.size() >= maximum_items) return;
    items.push_back({std::string(item_text), enabled});
    height += item_size.height;
    updateParentPosition();
    if (auto* owner = parent) owner->handleMessage(UiMessage::setSize, detail::wireWord(parent_width), detail::wireWord(parent_height + item_size.height));
}
...
void SphereUI::CMenuListControl::clearItems() {
    ...
    items.clear();
    hovered_index = maximum_items;
    if (auto* owner = parent) owner->handleMessage(UiMessage::setSize, detail::wireWord(parent_width), SferaNumeric::lowWord(parent_height - removed_height));
}
...
std::size_t SphereUI::CMenuListControl::itemAtPoint(std::int64_t point_x, std::int64_t point_y) const {
...
void SphereUI::CMenuListControl::drawHeader() {
    const auto bounds = windowBounds(*this);
    if (top_sprite != nullptr) top_sprite->drawNatural(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
    if (show_title) {
...
void SphereUI::CMenuListControl::drawItem(std::size_t index, const std::shared_ptr<const UiSprite>& sprite, std::uint32_t color) {
    if (index >= items.size()) return;
    const auto bounds = windowBounds(*this);
    const auto row_top = detail::addCoordinate(bounds.top + top_size.height, index * item_size.height);
    if (sprite != nullptr) sprite->drawNatural(SferaNumeric::real32(bounds.left), SferaNumeric::real32(row_top), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
...
void SphereUI::CMenuListControl::drawFooter() {
    const auto bounds = windowBounds(*this);
    if (bottom_sprite != nullptr) bottom_sprite->drawNatural(SferaNumeric::real32(bounds.left), SferaNumeric::real32(bounds.top + top_size.height + std::ssize(items) * item_size.height), SferaColor::rgba(255u, 255u, 255u, alpha).argb());
}
...
void SphereUI::RichEditCtrl::updateScroll() {
    ...
    parameters.maximum = detail::checkedInt(std::max(std::ptrdiff_t{}, std::ssize(lines) - page_rows - 1), "Rich edit scroll range exceeds int");
    parameters.page_step = detail::checkedInt(page_rows, "Rich edit page size exceeds int");
    parameters.current = detail::checkedInt(first_row, "Rich edit scroll position exceeds int");
    parameters.step = 1;
...
void SphereUI::RichEditCtrl::moveCaret(std::uint32_t key) {
    ...
    switch (key) {
        case VK_LEFT:
            ...
            else if (caret_row > 0) caret_column = std::ssize(lines.at(--caret_row));
            break;
        case VK_RIGHT:
...
void SphereUI::RichEditCtrl::setContent(std::string_view content_text) {
...
void SphereUI::RichEditCtrl::insertAt(std::ptrdiff_t column, std::string glyph, std::ptrdiff_t row) {
    ...
    for (;;) {
        ...
        if (line_text.size() < 2u || InterfaceRenderer::measureText(line_text, font, font_initialized).width + cursor_width <= width) {
...
std::ptrdiff_t SphereUI::RichEditCtrl::mergeRows(std::ptrdiff_t destination, std::ptrdiff_t source) {
    if (destination < 0 || source < 0 || destination >= std::ssize(lines) || source >= std::ssize(lines) || destination == source) return 0u;
    ...
    for (const auto character : following) {
        ...
        if (InterfaceRenderer::measureText(merged_text, font, font_initialized).width + cursor_width > width) {
...
void SphereUI::RichEditCtrl::eraseCharacter(bool backward) {
    ...
    if (backward && caret_column > 0) {
        ...
        if (row + 1 >= std::ssize(lines)) return;
...
void SphereUI::RichEditCtrl::drawCaret(int left, int top) {
    const auto now = Runtime::clockTicks();
    if (blink_started == 0u || now < blink_started) blink_started = now;
    const auto elapsed = now - blink_started;
    if (elapsed > 4000u) cursor_visible = (((elapsed - 4000u) / 4000u) & 1u) != 0u;
    if (!input_enabled || !cursor_visible || caret_row < 0 || caret_row >= std::ssize(lines)) return;
    ...
    InterfaceRenderer::drawText("_", left + extent.width, top, text_color, font, font_initialized, windowBounds(*this), false);
}
...
void SphereUI::RichEditCtrl::draw() {
    ...
    const auto bounds = windowBounds(*this);
    {
        ...
        if (viewport) {
            ...
            for (auto row = start; row < finish; ++row) InterfaceRenderer::drawText(lines.at(row), bounds.left, SferaNumeric::signedWord(SferaNumeric::lowWord(bounds.top + (row - start) * line_height)), text_color, font, font_initialized, bounds, false);
            if (caret_row >= start && caret_row < finish) drawCaret(bounds.left, SferaNumeric::signedWord(SferaNumeric::lowWord(bounds.top + (caret_row - start) * line_height)));
        }
    }
    drawChild(scrollbar.get(), alpha);
}
...
void SphereUI::HyperTextCtrl::queuePage(std::string_view page_name, bool remember) {
    if (transition_active) return;
    if (page_name.empty()) { pending_page = std::monostate{}; return; }
    PageRequest request{std::string(page_name.substr(0u, 255u)), remember};
...
void SphereUI::HyperTextCtrl::updateDocument(bool resize_to_content) {
    ...
    if (replacement != nullptr) {
        ...
        scroll_position = 0;
        updateScroll();
        if (auto* scroll = scrollbar.get()) scroll->setParameters({ScrollParameters::wire_size, ScrollField::all, 0, maximum_scroll, height, 0, document->line_height});
    }
...
void SphereUI::HyperTextCtrl::draw() {
    if (hidden) return;
    if (auto* tip = tooltip.get()) if (!tip->hidden && tip->alpha != alpha) tip->setOpacity(alpha / 255.0f);
    const auto bounds = windowBounds(*this);
    {
        ViewportScope viewport(bounds);
        if (viewport) {
            const auto fraction = std::clamp(transition_fraction, 0.0f, 1.0f);
            if (auto* page = previous_document.get()) page->draw(bounds.left, bounds.top - previous_line_offset, previous_clip_offset, previous_clip_height, SferaNumeric::truncatedWord((1.0f - fraction) * alpha));
            if (auto* page = document.get()) page->draw(bounds.left, bounds.top - first_line_offset, clip_offset, clip_height, SferaNumeric::truncatedWord(fraction * alpha));
        }
    }
    drawChild(scrollbar.get(), alpha);
}
...
void SphereUI::HyperTextCtrl::handleInput(const WindowInput& input) {
    ...
    for (std::size_t index = 0u; index < page->tooltips.size(); ++index) {
        ...
        if (region.hovered && hovered < 0) hovered = SferaNumeric::signedWord(SferaNumeric::lowWord(index));
    }
...
void SphereUI::HyperTextChatListControl::addMessage(std::string_view message_text, std::uint32_t channel, std::uint32_t color) {
...
void SphereUI::HyperTextChatListControl::updateScroll(bool reset) {
    ...
    if (auto* child = scrollbar.get()) child->setParameters({ScrollParameters::wire_size, ScrollField::all, 0, SferaNumeric::signedWord(SferaNumeric::lowWord(reset ? 0u : maximum)), 1, SferaNumeric::signedWord(SferaNumeric::lowWord(reset ? 0u : maximum - scroll_offset)), 1});
}
    ...
    const auto bounds = windowBounds(*this);
    {
        ...
        for (std::size_t item_index = bottom_message; item_index < visible_messages.size() && rendered_rows.size() < page_rows; ++item_index) {
            ...
            for (std::size_t reverse = last + 1u; reverse > first; --reverse) {
                ...
                const auto top = detail::subtractCoordinate(detail::subtractCoordinate(bounds.bottom, 1), (rendered_rows.size() + 1u) * row_height);
...
void SphereUI::HyperTextEditControl::setContent(std::string_view content_text, std::uint32_t mode) {
    if (mode > 1u) return;
    const auto source = content_text;
...
void SphereUI::HyperTextEditControl::insertPlainText(std::string_view plain_text) {
...
void SphereUI::HyperTextEditControl::drawCaret() {
    ...
    else if (cursor_type == 1u) InterfaceRenderer::drawTexture(nullptr, SferaNumeric::real32(left), SferaNumeric::real32(top + 2), SferaNumeric::real32(left + 1), SferaNumeric::real32(bounds.bottom - text_margins.bottom - 2), color, 0.0f, 0.0f, false);
}
...
void SphereUI::Window::alignToScreen(bool reset_position) {
    ...
    if ((alignment_flags & alignRight) != 0u) x = detail::addCoordinate(x, horizontal);
    if ((alignment_flags & alignBottom) != 0u) y = detail::addCoordinate(y, vertical);
    if ((alignment_flags & alignCenterX) != 0u) x = detail::addCoordinate(x, horizontal >> 1u);
    if ((alignment_flags & alignCenterY) != 0u) y = detail::addCoordinate(y, vertical >> 1u);
}
...
void SphereUI::Window::startAnimation(WindowAnimation::Phase phase) {
    ...
    if (effect.kind >= WindowAnimation::Kind::MoveLeft && effect.kind <= WindowAnimation::Kind::MoveDown) {
        double origin = effect.kind <= WindowAnimation::Kind::MoveRight ? x : y;
        origin -= (effect.kind == WindowAnimation::Kind::MoveRight || effect.kind == WindowAnimation::Kind::MoveDown) ? effect.distance : 0.0f;
...
void SphereUI::Window::updateAnimation() {
    if (!animation) return;
    const float elapsed = SferaNumeric::real32(Runtime::elapsedSeconds(animation->start_time));
...
SphereUI::Window* SphereUI::Window::controlAt(std::size_t index) {
    if (index == 0u) return this;
    return index <= children.size() ? children[index - 1u].get() : nullptr;
}

const SphereUI::Window* SphereUI::Window::controlAt(std::size_t index) const {
    if (index == 0u) return this;
    return index <= children.size() ? children[index - 1u].get() : nullptr;
}
...
namespace {
    template<class T> std::uint32_t vacantGuiSlot(const std::vector<std::unique_ptr<T>>& slots) {
    ...
    template<class Build> std::uint32_t createGuiControl(std::uint32_t windowHandle, std::string_view operation, Build&& build) {
        ...
        const std::uint32_t handle = vacantGuiSlot(WorldGuiControls::elements);
...
void SphereUI::FontFactory::loadConfiguration() {
    ...
    const int count = std::max(0, InterfaceConfiguration::readInteger("NEW_FONTS_NUMBER", 0));
...
int SphereUI::FontFactory::count() const noexcept {
...
const SphereUI::FontFace& SphereUI::FontFactory::face(int font) const {
    if (font < 2 || (font - 2) >= faces.size()) throw std::out_of_range("invalid font index");
...
bool SferaInterfaceCursor::loadTextures() {
    for (std::size_t index = 0u; index < textures.size(); ++index) {
        const auto name = sfera_cursor_texture_name(SphereUI::detail::wireWord(index));
...
void SferaInterfaceCursor::draw(float x, float y) const {
    ...
    if (kind < textures.size() && textures[kind] != 0) SphereUI::InterfaceRenderer::drawTexture(g_sfera_textures.resource(textures[kind]), x, y, x + 32.0f, y + 32.0f, UiColor::white);
    for (const auto& image : images) {
        if (image.texture == -1) continue;
        const float left = x + image.x, top = y + image.y;
        SphereUI::InterfaceRenderer::drawTexture(g_sfera_textures.resource(image.texture), left, top, left + image.width, top + image.height, UiColor::white);
    }
    ...
    for (const auto& label : labels) {
        ...
        SphereUI::InterfaceRenderer::drawText(label.text, SferaNumeric::truncateInt(label_origin_x + label.x), SferaNumeric::truncateInt(label_origin_y + label.y), label.color, label.font, true, clip, SferaColor::fromArgb(label.color).alpha() == 255u);
    }
}
...
SphereUI::CursorGeometry CCursorManager::geometry() const {
    ...
    const auto dimensions = active == &software ? SphereUI::TextExtent{software.texture_width, software.texture_height} : SphereUI::TextExtent{hardware.texture_width, hardware.texture_height};
...
namespace {
    float spriteTextureCoordinate(double pixel, int extent) {
...
void SphereUI::SpritePart::setTexture(std::string_view image_name, const UiRect* texture_rectangle, const UiRect* sprite_rectangle) {
    ...
    const float left = texture_rectangle == nullptr ? 0.0f : spriteTextureCoordinate(source.left, extent.width), top = texture_rectangle == nullptr ? 0.0f : spriteTextureCoordinate(source.top, extent.height), right = texture_rectangle == nullptr ? 1.0f : spriteTextureCoordinate(source.right, extent.width), bottom = texture_rectangle == nullptr ? 1.0f : spriteTextureCoordinate(source.bottom, extent.height);
...
void SphereUI::UiSprite::setImage(std::string_view image_name) {
...
bool SphereUI::UiSprite::loadUi(const std::string& filename, SferaSimpleParser& parser, const SferaParserRange& range) {
    ...
    while (parser.nextValue("tcoords")) {
        ...
        for (std::size_t vertex = 0u; vertex < 4u; ++vertex) {
            parts[part_index].u[vertex] = spriteTextureCoordinate(values[vertex * 2u + 1u], size.width);
            parts[part_index].v[vertex] = spriteTextureCoordinate(values[vertex * 2u + 2u], size.height);
        }
    }
    return true;
}
...
SphereUI::UiViewport SphereUI::InterfaceRenderer::viewport() {
    ...
    UiViewport result{};
    result.x = detail::checkedInt(value.X, "D3D viewport X exceeds UI coordinate range");
    result.y = detail::checkedInt(value.Y, "D3D viewport Y exceeds UI coordinate range");
    result.width = detail::checkedInt(value.Width, "D3D viewport width exceeds UI coordinate range");
    result.height = detail::checkedInt(value.Height, "D3D viewport height exceeds UI coordinate range");
...
void SphereUI::InterfaceRenderer::setViewport(const UiViewport& value) {
    if (value.x < 0 || value.y < 0 || value.width < 0 || value.height < 0) throw std::out_of_range("Negative D3D viewport");
...
void SphereUI::UiSprite::drawParts(float left, float top, float right, float bottom, std::uint32_t color, bool natural) const {
    ...
    for (std::size_t index = 0u; index < parts.size(); ++index) {
        ...
        const auto vertices = spriteQuad(part, left + position(part.rectangle.left, scale_x), top + position(part.rectangle.top, scale_y), left + position(part.rectangle.right, scale_x), top + position(part.rectangle.bottom, scale_y), color);
        (g_sfera_graphics_runtime.d3d_runtime->last_hresult = device->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX1));
        (g_sfera_graphics_runtime.d3d_runtime->last_hresult = device->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2u, vertices.data(), sizeof(SferaScreenVertex)));
    }
}

void SphereUI::UiSprite::drawRotated(float left, float top, float right, float bottom, std::uint32_t color, double rotation) const {
    if (parts.size() == 0u) return;
    ...
    auto vertices = spriteQuad(part, -sprite_width * 0.5f, -sprite_height * 0.5f, sprite_width * 0.5f, sprite_height * 0.5f, color);
...
void SphereUI::InterfaceManager::writeSavedPositions(std::span<std::byte> destination) const {
    ...
    for (const auto& position : saved_positions) {
        ...
        std::array<double, 2> pixels; pixels[0] = position.x; pixels[1] = position.y;
        std::array<double, 2> extent; extent[0] = g_sfera_graphics_runtime.display_width; extent[1] = g_sfera_graphics_runtime.display_height;
        std::array<float, 2> coordinates;
        std::transform(pixels.begin(), pixels.end(), extent.begin(), coordinates.begin(),
            [](double pixel, double dimension) { return SferaNumeric::real32(pixel / dimension); });
...
bool SphereUI::HyperTextParser::parseCommand(std::string_view tag, HyperDocumentNode& node, std::string& argument) {
    ...
    if (node.command == HyperTextCommand::color) node.color = std::strtoul(argument.c_str(), nullptr, 16) | D3DCOLOR_XRGB(0, 0, 0);
...
std::unique_ptr<SphereUI::HyperTextDocument> SphereUI::HyperTextDocument::clone(int width, std::uint32_t format, int font_id) const {
...
void SphereUI::HyperTextDocument::layout(int width, std::uint32_t format, int font_id) {
    width = std::max(width, 1);
    const auto tracking = InterfaceRenderer::tracking(font_id);
    const auto space = hyperTextExtent(" ", font_id);
    ...
    for (const auto& original : originals) {
        ...
        while (start < text.size()) {
            ...
            const auto full_extent = hyperTextExtent(remaining, font_id);
            ...
            const auto extent = hyperTextExtent(word, font_id);
            auto visible = word;
            if (rows.back().width != 0 && rows.back().width + extent.width > width) {
                ...
                if (hyperTextExtent(text.substr(start), font_id).width <= width) continue;
            }
            if (!visible.empty()) {
                auto& current = rows.back();
                ...
                current.width += hyperTextExtent(visible, font_id).width + tracking;
            }
            start = end;
        }
    }
    ...
    for (auto& row : rows) {
        ...
        for (auto& fragment : row.fragments) {
            fragment.node.width = hyperTextExtent(fragment.text, font_id).width;
    ...
    for (std::size_t index = 0u; index < rows.size(); ++index) {
        ...
        for (auto& fragment : row.fragments) {
            ...
            while (start < fragment.text.size()) {
                ...
                node.width = hyperTextExtent(text, font_id).width + (end < fragment.text.size() ? tracking : 0);
                result.fragments.push_back({node, text});
                if (!text.empty() && text.back() == ' ' && text.find_first_not_of(' ') != std::string::npos) {
                    const auto added = ++used_gaps == gaps ? spare - distributed : std::cmp_greater(gaps, spare) ? 0 : spare / detail::checkedInt(gaps, "hypertext gap count exceeds int");
                    HyperDocumentNode padding{};
                    padding.command = HyperTextCommand::horizontalSpace;
...
int SphereUI::HyperTextDocument::totalHeight() const {
    ...
    return detail::checkedInt(std::clamp<std::int64_t>(height, 0, std::numeric_limits<int>::max()), "hypertext height exceeds int");
}
...
void SphereUI::HyperTextDocument::draw(int left, int top, int clip_offset, int clip_height, std::uint32_t alpha) {
    ...
    const UiRect clip{viewport.x, std::max(top, viewport.y), viewport.x + viewport.width, std::min(top + std::max(clip_height, 0), viewport.y + viewport.height)};
    for (auto* group : {&links, &tooltips}) for (auto& region : *group) {
        region.rectangles.clear();
        region.visible = false;
    }
    ...
    for (const auto& line : lines) {
        ...
        for (const auto& node : line.nodes) {
            if (node.command == HyperTextCommand::color) color = node.color;
                ...
                if (node.sprite != nullptr && y + line.height > clip.top && y < clip.bottom) node.sprite->drawNatural(SferaNumeric::real32(x + node.image_x), SferaNumeric::real32(y + node.image_y), SferaColor::rgba(255u, 255u, 255u, opacity).argb());
                x += node.image_width;
            }
...
void SphereUI::HyperTextChatListItem::layout(int width, int font) {
    ...
    const auto wrap = [&](std::string_view separator) {
        ...
        first = detail::wireWord(output.size());
        x = 0;
    };
...
void SphereUI::InterfaceManager::collectCoveredWindows(Window& window) {
    ...
    for (auto current = windows.rbegin(); current != windows.rend(); ++current) {
        ...
        const double covered_area = detail::real64(covered_width * covered_height);
        const float coverage = SferaNumeric::real32(covered_area / detail::real64(area));
        if (coverage > 0.3f) window.addModalReference(*other);
    }
}
...
void SphereUI::InterfaceManager::draw() {
    ...
    try {
        ...
        if (cursor && cursor->system_visible) {
            ...
            SferaCursorPosition position{};
            active->getPosition(&position);
            if (active->isInsideViewport()) cursor->draw(SferaNumeric::real32(position.x), SferaNumeric::real32(position.y));
        }
...
void SphereUI::InterfaceManager::showLoadingScreen(bool visible, int width, int height, bool english) {
    ...
    if (auto* image = load_screen->controlAt(2u)->asImageCtrl()) { ImageDescription description{}; description.name = english ? "english_sphere1" : "russian_sphere1"; image->setImage(&description); }
...
void SphereUI::InterfaceManager::setLoadingProgress(int percent) {
    if (load_screen != nullptr) sendMessage(load_screen->controlAt(3u), UiMessage::setProgressValue, std::clamp(percent, 0, 100), 0u);
}
...
void SphereUI::InterfaceManager::update(std::uint8_t key, std::uint8_t character, std::uint32_t mouse_buttons, int wheel_delta) {
    ...
    if (drag_drop_active) {
        ...
        if ((input.mouse_flags & MouseInput::leftRelease) != 0u) {
            ...
            if (!accepted) queueEvent({nullptr, 0u, UiMessage::dropToScene, detail::wireWord(position.x), detail::wireWord(position.y)});
            for (const auto& owner : windows) if (auto* window = owner.get()) {
...
bool SphereUI::InterfaceManager::isRegistered(const Window* window) const { return window != nullptr && registered_windows.contains(window); }
...
void SphereUI::InterfaceManager::unbindEventHandler(const Window* window) { event_handlers.erase(window); }
...
void SphereUI::InterfaceConfiguration::save() {
    if (!filename_.empty()) SferaFileManager::writeFile(filename_, std::as_bytes(std::span(text_)));
}
...
namespace {
    ...
    void uiFontTriangles(std::uint16_t* indices, std::uint32_t first, bool clockwise) {
        ...
        for (std::uint32_t index = 0u; index < 6u; ++index) indices[index] = SferaNumeric::lowHalf(first + order[index]);
    }
    ...
    void uiDrawAtlasText(std::string_view text, int x, int top, std::uint32_t color, int font, int scale, const SphereUI::UiRect& clip, float depth, bool configureAlpha) {
        ...
        for (const std::uint8_t byte : text) {
            ...
            x = SphereUI::detail::addCoordinate(x, (g_sfera_font_runtime.faces[font].widths[character] + g_sfera_font_runtime.faces[font].code_base) * scale);
        }
        flush();
...
SphereUI::TextExtent SphereUI::InterfaceRenderer::measureText(std::string_view text, int font, bool initialized) {
    ...
    for (const std::uint8_t byte : text) {
        if (byte == '\n') {
            ...
            if (SferaNumeric::signedWord(completed_width) > SferaNumeric::signedWord(width)) width = completed_width;
            if (++lines == 300u) throw std::length_error("too many lines in font measurement");
            line_width = 0u;
        } else {
            if (SferaNumeric::signedWord(line_width) > 0) line_width += SferaNumeric::word(face.code_base);
            line_width += SferaNumeric::word(g_sfera_font_runtime.faces[font].widths[uiAtlasGlyph(byte)]);
        }
    }
    if (SferaNumeric::signedWord(line_width) > SferaNumeric::signedWord(width)) width = line_width;
...
void SphereUI::InterfaceRenderer::drawText(std::string_view text, int x, int y, std::uint32_t color, int font, bool initialized, const UiRect& clip, bool opaque) {
    ...
    if (font < 2u) uiDrawAtlasText(text, x, SphereUI::detail::subtractCoordinate(SphereUI::detail::subtractCoordinate(y, 2), g_sfera_font_runtime.faces[font].origin), color, font, initialized ? 1 : 0, clip, 0.0f, false);
...
namespace {
    ...
    const std::array<std::array<std::uint8_t, 256>, 2>& chatCharacterMaps() {
        static const auto maps = [] {
            ...
            const auto aliases = [&](std::size_t alphabet, std::string_view group) { for (std::size_t alias = 1; alias < group.size(); ++alias) for (auto& value : result[alphabet]) if (value == SferaText::byteValue(group[alias])) value = group.front(); };
            for (auto group : {"\345\270e", "\344g", "\341" "6b", "\363y", "\352k", "\343r", "\347" "3", "\365x", "\350\351u", "\356" "0o", "\360p", "\361c", "\340a", "\354m", "\355h"}) aliases(0u, group);
...
SphereUI::ChatFilter::ChatFilter() {
    ...
    for (std::size_t index = 0u; index < *count; ++index) {
        ...
        Rule rule{std::string(*word), *kind};
...
std::string SphereUI::ChatFilter::normalize(std::string_view text, std::size_t alphabet) {
    ...
    for (auto& value : result) value = map[SferaText::byteValue(value)];
    return result;
}
...
bool SphereUI::ChatFilter::matchesWord(std::string_view word, std::size_t alphabet) const {
    for (const auto& rule : rules) {
        ...
        switch (rule.kind) {
            ...
            case 3u: if (position != 0u && !chatRussianVowelOrSign(SferaText::byteValue(word[position - 1u]))) continue; break;
            case 4u: if (position != 0u || (word.size() != pattern.size() && !chatRussianVowelOrSign(SferaText::byteValue(word[pattern.size()])))) continue; break;
            default: continue;
        }
        bool excepted = false;
...
void GameInterface::drawFullscreenOverlay() {
    ...
    auto& device = *g_sfera_graphics_runtime.d3d_runtime;
    auto* vertices = SceneSky::sun_quad;
    interfaceQuad(vertices, 0.0f, 0.0f, SferaNumeric::real32(g_sfera_graphics_runtime.display_width), SferaNumeric::real32(g_sfera_graphics_runtime.display_height), SferaColor::rgba(255u, 255u, 255u, alpha).argb(), nullptr, 0.0f, 1.0f, true);
...
int GameInterface::fontHeight(int font, int scale) { return g_sfera_font_runtime.faces[font].span * scale; }
int GameInterface::glyphWidth(std::uint32_t character, int font) { return g_sfera_font_runtime.faces[font].widths[character] + g_sfera_font_runtime.faces[font].code_base; }
int GameInterface::textHeight(int font, int scale, int lines) { return ((lines - 1) * g_sfera_font_runtime.faces[font].cell_step + g_sfera_font_runtime.faces[font].span) * scale; }
int GameInterface::lineOffset(int font, int scale, int line) { return g_sfera_font_runtime.faces[font].cell_step * scale * line; }
...
void GameInterface::updateLoadingProgress(int increment) {
    ...
    if (progress == g_sfera_graphics_runtime.rebuild_percent) return;
...
namespace {
    ...
    void interfaceElement(const GameUiElement& element, int scrollX, int scrollY) {
        ...
        if (element.objectHandle != UINT32_MAX) {
            ...
            if (g_sfera_camera.near_distance > depth) { const double near_distance = g_sfera_camera.near_distance; constexpr double depth_epsilon = 0.01f; depth = SferaNumeric::real32(near_distance + depth_epsilon); }
        }
...
void GameInterface::drawWindow(std::uint32_t handle) {
    ...
    auto& device = *g_sfera_graphics_runtime.d3d_runtime;
    const D3DVIEWPORT9 viewport{SferaNumeric::word(window->contentLeft), SferaNumeric::word(window->contentTop), SferaNumeric::word(window->contentRight - window->contentLeft + 1), SferaNumeric::word(window->contentBottom - window->contentTop + 1), 0, 1};
...
std::vector<std::uint32_t> GameInterface::orderedWindows(WindowOrder order) {
    std::vector<std::uint32_t> result;
    result.reserve(windows.size());
    for (std::uint32_t handle = 0; handle < windows.size(); ++handle)
        if (window(handle)) result.push_back(handle);
    SferaAlgorithms::stableSort(result, [order](std::uint32_t handle) {
        const auto& item = *window(handle);
        const std::uint32_t bits = item.layer * 1000u + item.order;
        return SferaNumeric::signedWord(order == WindowOrder::HitTest ? 0u - bits : bits);
    });
    return result;
}

void GameInterface::drawAll() {
    for (std::uint32_t handle : orderedWindows(WindowOrder::Draw)) drawWindow(handle);
}
...
std::uint32_t GameInterface::createWindow(int left, int top, int right, int bottom, std::uint32_t layer, std::uint32_t opacity) {
    ...
    created->layer = layer;
    created->order = SferaNumeric::lowWord(std::count_if(windows.begin(), windows.end(), [](const auto& item) { return item != nullptr; }));
...
std::uint32_t GameUiWindow::attach(std::uint32_t control) {
    ...
    if (slot != controls.end()) { *slot = control; return SphereUI::detail::wireWord(slot - controls.begin()); }
    if (controls.size() >= 7000) WorldDiagnostics::fail("MNO_CONTROLS_IN_WINDOW exceed");
    controls.push_back(control);
    return SphereUI::detail::wireWord(controls.size() - 1u);
}
...
void GameUiWindow::scrollBy(float dx, float dy, SferaCursorPosition& cursor) {
    ...
    const auto clampAxis = [](float& scroll, int limit, int& position) {
        if (scroll < 0.0f) { const double precise_position = position; position = SferaNumeric::truncateInt(precise_position + scroll); scroll = 0.0f; }
        const float maximum = SferaNumeric::real32(limit);
        if (scroll > maximum) { const double precise_scroll = scroll; position = SferaNumeric::truncateInt(precise_scroll - maximum + position); scroll = maximum; }
    };
...
void GameUiElement::layoutText(std::string_view text, const GameUiWindow& window, int x, int y) {
    ...
    const int textHeight = GameInterface::textHeight(
        layout.font, layout.font_scale, SphereUI::detail::checkedInt(layout.lines.size(), "Text layout line count exceeds int"));
...
GameUiHit GameInterface::hitTest(SferaCursorPosition point) {
    GameUiHit hit{};
    for (std::uint32_t handle : orderedWindows(WindowOrder::HitTest)) {
        const auto& item = *window(handle);
...
void GameFontAtlas::load(int font, const std::string& filename, int outline, int spacing, int emptyWidth) {
    if (font < 0 || font >= faces.size() || outline < 0 || outline > 3) WorldDiagnostics::fail("Invalid bitmap font parameters");
    ...
    for (std::size_t position = 0; (position = mapping.find('#', position)) != std::string_view::npos;) {
        ...
        int rightmost = 0;
        for (int y = 0; y < sourceCell; ++y) for (int x = 0; x < sourceCell; ++x) {
        ...
        placements[character] = {SphereUI::detail::wireWord(symbols / 64u), 1.0f * (tileX + 3 - outline) / atlasWidth, 1.0f * (tileY + 3 - outline) / atlasWidth, true};
...
std::uint32_t SphereUI::Runtime::scanCode(std::uint32_t key) {
        ...
        for (const auto& row : rows) {
            const auto position = std::find_if(row.keys.begin(), row.keys.end(),
                [key](char candidate) { return SferaNumeric::word(candidate) == key; });
            if (position != row.keys.end()) return row.first_scan + SphereUI::detail::wireWord(position - row.keys.begin());
        }
...
std::uint32_t SphereUI::Runtime::virtualKey(std::uint32_t scan) {
        for (std::uint32_t key = 0u; key <= 255u; ++key) if (SphereUI::Runtime::scanCode(key) == scan) return key;
        return 0u;
    }
...
void CHardwareCursor::setPosition(int x, int y) { if (!SferaClientApplication::application_active) return; POINT point{x, y}; ::ClientToScreen(SferaClientApplication::main_window, &point); ::SetCursorPos(point.x, point.y); }
...
void CSoftwareCursor::updatePosition() { if (!active) return; x += SferaNumeric::truncateInt(g_sfera_direct_input_runtime.mouse.dx * 1.5); y += SferaNumeric::truncateInt(g_sfera_direct_input_runtime.mouse.dy * 1.5); }
